...
<h1 class="py-5 text-lg font-semibold"> <span class="font-bold">{data.username}</span>'s  Participated Contests</h1>
		...
		<Table.Root class="lg:text-lg">
			<Table.Header>
				<Table.Row>
					...
					<Table.Head class="text-right">Team name</Table.Head>
				</Table.Row>
			</Table.Header>
			<Table.Body>
				{#each data.contest_list as contest (contest)}
					...
					>
						...
						<Table.Cell>{ifNull(contest.solved)}</Table.Cell>
						<Table.Cell class="text-right">{contest.team_name}</Table.Cell>
