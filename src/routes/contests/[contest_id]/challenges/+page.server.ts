...
export const load = async ({url,fetch,params,locals}) => {
    ...
    else if(contest_access['contest_status'] == 'upcoming' || contest_access['contest_paused'] == true) {
        // none
        return {
            challenge_list : [],
            contest_id,
            contetst_status : contest_access['contest_status'],
            contest_paused : contest_access['contest_paused']
        };
    }
